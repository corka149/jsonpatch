defmodule Jsonpatch do
  ...
  def diff(source, destination, opts \\ []) do
    ...
    cond do
      is_map(source) and is_map(destination) -> do_map_diff(destination, source, opts)
      is_list(source) and is_list(destination) -> do_list_diff(destination, source, opts)
      true -> []
    end
  end
  ...
  defp do_list_diff(destination, source, ancestor_path \\ "", patches \\ [], opts) do
    if opts[:object_hash] do
      ...
      do_pairwise_list_diff(destination, source, ancestor_path, patches, opts)
    end
  catch
    # happens if we've got a nil hash or we tried to hash a non-map
    :nil_hash -> do_pairwise_list_diff(destination, source, ancestor_path, patches, 0, opts)
  end

  defp do_pairwise_list_diff(destination, source, ancestor_path, patches, idx \\ 0, opts)
  ...
  defp do_pairwise_list_diff(items, [], path, patches, idx, _opts) do
    # we have to do it without recursion, because we have to keep the order of the items
    items
    |> Enum.map_reduce(idx, fn val, idx ->
      {%{op: "add", path: "#{path}/#{idx}", value: val}, idx + 1}
    end)
    |> elem(0)
    |> Kernel.++(patches)
  end

  defp do_pairwise_list_diff([val | rest], [source_val | source_rest], path, patches, idx, opts) do
    # case when there's an item in both desitation and source. Let's just compare them
    patches = do_diff(val, source_val, path, idx, patches, opts)
    do_pairwise_list_diff(rest, source_rest, path, patches, idx + 1, opts)
  end
